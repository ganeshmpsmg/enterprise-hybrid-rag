...
class TestEmbeddingModel:
    ...
    def test_embed_batch_shape(self, mock_model):
        mock_model._model.encode.return_value = np.random.rand(3, 384).astype(np.float32)
        result = mock_model.embed_batch(["text1", "text2", "text3"])
        assert result.shape == (3, 384)

    def test_caching(self, mock_model):
        mock_model._model.encode.return_value = np.random.rand(1, 384).astype(np.float32)
